...
int select_priority(Process p[], int n, double time_now) {
    int idx = -1;
    int best_pr = INT_MAX;

    for (int i = 0; i < n; i++) {
        if (!p[i].done && p[i].at <= time_now) {
            if (p[i].pr < best_pr) {
                best_pr = p[i].pr;
                idx = i;
...
int main() {
    ...
    printf("ATM Monitoring and Alert System - Priority Scheduler (Linux)\n");
    printf("(Non-Preemptive, lower number = higher priority)\n");
    ...
    for (int i = 0; i < n; i++) {
        if (p[i].at < first_arrival) first_arrival = p[i].at;
    }

    while (completed < n) {
        int idx = select_priority(p, n, time_now);
    ...
    printf("\n+-----+------+------+------+--------+--------+--------+--------+--------+\n");
    printf("| PID |  AT  |  BT  | PRIO | Start  | Finish |   WT   |  TAT   |   RT   |\n");
    printf("+-----+------+------+------+--------+--------+--------+--------+--------+\n");
    for (int i = 0; i < n; i++) {
        printf("| %3d | %4.1f | %4.1f | %4d | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f |\n",
               p[i].pid, p[i].at, p[i].bt, p[i].pr, p[i].st, p[i].ft, p[i].wt, p[i].tat, p[i].rt);
    }
    printf("+-----+------+------+------+--------+--------+--------+--------+--------+\n");
