...
int main() {
    ...
    printf("ATM Monitoring and Alert System - Round Robin Scheduler (Linux)\n");
    ...
    while (completed < n) {
        if (front == back) {
            ...
            int next_idx = -1;
            for (int i = 0; i < n; i++) {
                if (!p[i].done && p[i].at < next_at) { next_at = p[i].at; next_idx = i; }
            }
            time_now = next_at;
            if (next_idx != -1 && !inq[next_idx]) { q[back++] = next_idx; inq[next_idx] = 1; }
            continue;
        }
        ...
        double slice = (p[idx].rem > Q) ? Q : p[idx].rem;
        time_now += slice;
        p[idx].rem -= slice;
        total_busy += slice;

        for (int i = 0; i < n; i++) {
            if (!p[i].done && !inq[i] && p[i].at <= time_now) {
                q[back++] = i; inq[i] = 1;
            }
        }
